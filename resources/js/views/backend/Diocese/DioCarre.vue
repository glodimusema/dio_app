            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_carre"></v-text-field>
                                  
                  <v-autocomplete label="Selectionnez le Quartier" prepend-inner-icon="mdi-map" dense
                      :rules="[(v) => !!v || 'Ce champ est requis']" :items="ParoisseList" item-text="nom_communaute" item-value="id"
                      outlined v-model="svData.refCommunaute">
                  </v-autocomplete>
...
export default {
  ...
  methods: {
    ...
    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout Carré";
      this.edit = false;
    ...
    //   }, 300),
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_dio_carre?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_dio_carre`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_dio_carre/${id}`).then(
        ({ data }) => {
          var donnees = data.data;

          donnees.map((item) => {
            this.titleComponent = "modification de " + item.nom_carre;
          });
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_dio_carre/${id}`).then(
          ({ data }) => {

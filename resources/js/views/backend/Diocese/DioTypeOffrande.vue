            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_type_offrande"></v-text-field>

                  <v-text-field label="Montant" prepend-inner-icon="extension" dense
...
export default {
  ...
  data() {
    return {
      ...
      disabled: false,
      edit: false,
      //'id','nom_type_offrande','montant','devise','taux','author','refUser','active'
      svData: {
        id: "",
        nom_type_offrande: "",
        montant: "",
  ...
  methods: {
    ...
    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout Type Offrande";
      this.edit = false;
    ...
    //   }, 300),
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_dio_type_offrande?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_dio_type_offrande`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_dio_type_offrande/${id}`).then(
        ({ data }) => {
          var donnees = data.data;

          donnees.map((item) => {
            this.titleComponent = "modification de " + item.nom_type_offrande;
          });
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_dio_type_offrande/${id}`).then(
          ({ data }) => {

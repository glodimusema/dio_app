                  ...
                  <v-dialog v-model="dialog" max-width="700px" persistent>
                    <v-card :loading="loading">
                      <v-form ref="form" lazy-validation>
                        <v-card-title>
                          Details Offrandes <v-spacer></v-spacer>
                        ...
                        <v-card-text>
                          ...
                          <v-layout row wrap>
                            ...
                            <v-flex xs12 sm12 md12 lg12>
                              <div class="mr-1">
                                <v-autocomplete label="Selectionnez le Chretien" prepend-inner-icon="mdi-map"
                                  :rules="[(v) => !!v || 'Ce champ est requis']" :items="chretienList" item-text="noms_chretien"
                                  item-value="id" dense outlined v-model="svData.refChretien" chips clearable >
                                </v-autocomplete>
                              </div>
                            </v-flex>
                            ...
                            <v-flex xs12 sm12 md6 lg6>
                              <div class="mr-1">
                                <v-autocomplete label="Selectionnez le Type d'Offrande" prepend-inner-icon="mdi-map"
                            ...
                            <v-flex xs12 sm12 md6 lg6>
                              <div class="mr-1">
                                <v-text-field type="number" label="Montant Offrande" prepend-inner-icon="event" dense
                                  outlined v-model="svData.montant_offrande">
                                </v-text-field>
                              </div>
                            </v-flex>
                            <v-flex xs12 sm12 md6 lg6>
                            ...
                            <v-flex xs12 sm12 md6 lg6>
                              <div class="mr-1">
                                <v-text-field type="date" label="Date Offrande" prepend-inner-icon="event" dense
                                  outlined v-model="svData.date_offrande">
                                </v-text-field>
                              </div>
                            </v-flex>
                            <v-flex xs12 sm12 md6 lg6>
                            ...
                            <v-flex xs12 sm12 md12 lg12>
                              <div class="mr-1">
                                <v-text-field label="Autres Details" prepend-inner-icon="event" dense
                                  outlined v-model="svData.autres_details_offrende">
                                </v-text-field>
                              </div>
                            </v-flex>
                                ...
                                <tr v-for="item in fetchData" :key="item.id">
                                  ...
                                  <td>
                                    ...
                                    <v-tooltip  top color="black">
                                      <template v-slot:activator="{ on, attrs }">
                                        <span v-bind="attrs" v-on="on">
                                          <v-btn @click="printBill(item.refChretien)" fab small><v-icon
        ...
        validate() {
          if (this.$refs.form.validate()) {
            ...
            if (this.edit) {
              ...
              this.insertOrUpdate(
                `${this.apiBaseURL}/update_dio_offrandes/${this.svData.id}`,
              ...
              this.svData.refUser = this.userData.id;
              this.insertOrUpdate(
                `${this.apiBaseURL}/insert_dio_offrandes`,
                JSON.stringify(this.svData)
              )
                .then(({ data }) => {
                  this.showMsg(data.data);

            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_categorie_paroisse"></v-text-field>

                  <v-text-field label="Code" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.code_categorie_paroisse"></v-text-field>
              ...
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="item in fetchData" :key="item.id">
                    ...
                    <td>{{ item.code_categorie_paroisse }}</td>
                    <td>{{ item.author }}</td>                    
                    <td>{{ item.active }}</td>
                    <td>
...
export default {
  ...
  data() {
    return {
      ...
      disabled: false,
      edit: false,
      //'id','nom_categorie_paroisse','code_categorie_paroisse','author','refUser','active'
      svData: {
        id: "",
        nom_categorie_paroisse: "",
        code_categorie_paroisse: "",
        author: "",
        refUser:0,
        active: ""
      },
  ...
  methods: {
    ...
    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout Categorie Paroisse";
      this.edit = false;
    ...
    //   }, 300),
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_dio_categorie_paroisse?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_dio_categorie_paroisse`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_dio_categorie_paroisse/${id}`).then(
        ({ data }) => {
          var donnees = data.data;

          donnees.map((item) => {
            this.titleComponent = "modification de " + item.nom_categorie_paroisse;
          });
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_dio_categorie_paroisse/${id}`).then(
          ({ data }) => {

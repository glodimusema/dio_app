            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_type_sacrement"></v-text-field>

                  <v-text-field label="Code" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.code_type_sacrement"></v-text-field>

                  <v-text-field label="Qualification" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.qualification_type_sacrement"></v-text-field>
              ...
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="item in fetchData" :key="item.id">
                    <td>{{ item.nom_type_sacrement }}</td>
                    <td>{{ item.code_type_sacrement }}</td>
                    <td>{{ item.qualification_type_sacrement }}</td>
                    <td>{{ item.author }}</td>                    
                    <td>{{ item.active }}</td>
                    <td>
...
export default {
  ...
  methods: {
    ...
    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout Type Sacrement";
      this.edit = false;
    ...
    //   }, 300),
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_dio_type_sacrement?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_dio_type_sacrement`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_dio_type_sacrement/${id}`).then(
        ({ data }) => {
          var donnees = data.data;

          donnees.map((item) => {
            this.titleComponent = "modification de " + item.nom_type_sacrement;
          });
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_dio_type_sacrement/${id}`).then(
          ({ data }) => {
